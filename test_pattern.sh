if [ $1 == 0 ]; then
   echo "Disable pattern generation"

   # VTX0
   # BIT[7-5] = 0 Disable generating VS, HS, DE in free running mode BIT[1-0] = 3
   echo "0x1c8 0x03" > /sys/class/i2c-dev/i2c-1/device/1-0064/regdump


   # VTX29
   # PATTERNGEN_MODE
   # Gradient pattern generation disable
   echo "0x1e5 0x00" > /sys/class/i2c-dev/i2c-1/device/1-0064/regdump

   exit
fi;

echo "Generating test pattern using HDMI input clock. DE/HS/VS is generated based on pixel lock in fee running mode"

# VTX0
# BIT[7-5] = 1 Generate VS, HS, DE in free running mode BIT[1-0] = 3
echo "0x1c8 0xe3" > /sys/class/i2c-dev/i2c-1/device/1-0064/regdump

# VTX1
# Don't touch, make it default (0xe1)
# BIT 4: 0-use clock from HDMI input, 1-use GPIO4 as PCLK
...
# VTX29
# PATTERNGEN_MODE
# Gradient pattern generation enable
echo "0x1e5 0x02" > /sys/class/i2c-dev/i2c-1/device/1-0064/regdump
